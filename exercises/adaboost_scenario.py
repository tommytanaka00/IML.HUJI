...
def fit_and_evaluate_adaboost(noise, n_learners=250, train_size=5000, test_size=500):
    ...
    for i in range(n_learners):
        decision_stump = DecisionStump()
        adaboost = AdaBoost(lambda :decision_stump, i+1)
        adaboost.fit(train_X, train_y)
    ...
    for val,i in enumerate(T):
        ...
        fig.add_traces([decision_surface(adaboost.fit(train_X,train_y).partial_predict(test_X, val), lims[0], lims[1], showscale=False),
